

# 📊 Quick Commerce: Sales & Delivery Intelligence Dashboard

### *Empowering Data-Driven Quick Commerce Decisions*

Developed for the **Microsoft Elevate AICTE Internship Program (Feb 2026 Batch)**

![Power BI](https://img.shields.io/badge/POWER%20BI-yellow?style=for-the-badge&logo=powerbi)
![Data Analytics](https://img.shields.io/badge/DATA%20ANALYTICS-blue?style=for-the-badge)
![Microsoft Elevate](https://img.shields.io/badge/MICROSOFT%20ELEVATE-blue?style=for-the-badge)
![AICTE](https://img.shields.io/badge/AICTE-orange?style=for-the-badge)

## 👨‍💻 Intern Details

| Attribute | Details |
|----------|--------|
| Intern Name | Mohammed Abdul Rahman |
| University / Institute | JNTUH University College Of Engineering Palair |
| AICTE Student ID | STU6976153e7cc851769346366 |
| Student Enrollment Number | 23XY1A0524 |
| Internship Program | Microsoft Elevate AICTE Internship |
| Domain | Data Analytics |
| Tools Used | Microsoft Power BI, Excel |
| Duration | 16 Feb 2026 – 12 March 2026 |

Quick Commerce: Sales & Delivery Intelligence Dashboard
Empowering Data-Driven Quick Commerce Decisions
Developed for the Microsoft Elevate AICTE Internship Program (Feb 2026 Batch)



📌 Project Overview
The Quick Commerce Sales and Delivery Intelligence Dashboard provides insights into sales performance, customer payment preferences, delivery efficiency, and order trends.
This dashboard helps businesses monitor key metrics and make data-driven decisions using interactive visualizations built in Power BI.

📊 Dashboards Included
1️⃣ Overview Dashboard
Provides a summary of key business metrics:
Total Orders
Total Revenue
Total Quantity Sold
Average Delivery Time
2️⃣ Sales Analysis Dashboard
Analyzes product and city-wise sales performance.
3️⃣ Payment Insights Dashboard
Shows customer payment preferences such as UPI, Card, and Cash.
4️⃣ Delivery Performance Dashboard
Evaluates delivery efficiency across different cities.
5️⃣ Order Trend Dashboard
Tracks order and revenue trends over time.

Key Insights
- UPI is the most used payment method.
- Mumbai has the highest number of orders.
- Grocery category generates the highest revenue.

 Tools & Technologies
Microsoft Power BI
Microsoft Excel
Data Visualization
Business Intelligence

## 📷 Dashboard Preview

### Overview Dashboard
![Overview Dashboard](overviewdashboard.png)

### Sales Analysis Dashboard
![Sales Analysis](SalesAnalysis.png)

### Payment Insights Dashboard
![Payment Insights](paymentinsights.png)

### Delivery Performance Dashboard
![Delivery Dashboard](deliverydashboard.png)

### Order Trend Dashboard
![Order Trend](orderanalysis.png)


 Conclusion:
This project demonstrates how Power BI dashboards can transform raw quick commerce data into actionable business insights, enabling organizations to improve operational efficiency and customer satisfaction.

Developed By:
Mohammed Abdul Rahman,
Microsoft Elevate AICTE Internship Project

LinkedIn profile-https://www.linkedin.com/in/mohammed-abdul-rahman-9739223a2?utm_source=share&utm_campaign=share_via&utm_content=profile&utm_medium=android_app

